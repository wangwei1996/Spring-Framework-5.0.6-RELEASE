...
public class WelcomeServiceImpl$$EnhancerBySpringCGLIB$$d24c71 extends WelcomeServiceImpl implements SpringProxy, Advised, Factory
{
    ...
    public final String sayHello(final String s) {
        try {
            ...
            /* 当MethodInterceptor存在的时候，则调用其intercept方法，参数依次为: 代理对象，正在执行的方法(被代理类的方法),方法参数,代理方法
             ...
             *
             * 
             *  这里为什么可以实现AOP？请参考:org.springframework.aop.framework.CglibAopProxy.DynamicAdvisedInterceptor,这是一个内部类
             */
...
```
### 02-4. MethodProxy代码
+ 不论是MethodProxy中的invoke方法被调用还是invokeSuper方法被调用，其实开始调用的地方都是代理类的对应的方法，只不过是MethodInterceptor中调用的选择不一致。
+ 问题: 为什么调用MethodInterceptor的时候，不会栈溢出?
   - 因为调用路径如下:
       ...
       4. invokeSuper方法又调用了代理类的方法，请注意，此时就不是调用的代理类的sayHello方法了，而是代理类中的CGLIB$sayHello$0方法(可通过打断点的方式观察)
...
public class MethodProxy {
    ...
    /**
      ...
      * 通过反编译过的FastClass可以看出,fci.f1.invoke 就是调用的是被代理类的方法
      *
      */
    ...
    /**
      ...
      * 通过反编译过的FastClass可以看出,fci.f1.invoke 就是调用的是代理类的方法
      *
      */
